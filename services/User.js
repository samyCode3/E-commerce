...
const register = async(req, res) => {
  ...
  await User.findOne({email: email.toLowerCase()}).then((data) => {
    ...
    MyUser.save().then((data) => {
      ...
      data.token = token
        return res.status(201).json({status:"Success-message", message: "User was successfully registered",  token: token})
    })
  }).catch(err => {
  ...
  const forgotPassword = (req, res) => {
    ...
    User.findOne({email}, (err, user) => {
     ...
     return User.updateOne({resetLink: token}).then((data) => {
        if(!data) return res.status(400).json({message: "Password Link is invaild"})
  ...
  const ResetPassword = async(req, res) => {
      ...
      if(resetLink)
      {
        jwt.verify(resetLink, process.env.FORGET_PASSWORD_TOKEN, (err, data) =>{
           if(err) return res.status(401).json({message: "Incorrect token or Token is expired"})
           User.findOne({resetLink}, (err, user) => {
            if(err || !user) return res.status(401).json({message: "User with this token does not exist."})
            const obj = 
            {
              password: NewPassword,
              resetLink: ''
